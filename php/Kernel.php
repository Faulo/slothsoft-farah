...
use Slothsoft\Farah\Module\Repository;
use Slothsoft\Farah\Module\AssetUri;
use Slothsoft\Farah\Module\Module;
use Slothsoft\Farah\Tracking\Manager;
use Slothsoft\PT\Repository as PTRepository;
use DOMDocument;
use DOMElement;
...
class Kernel
{
    ...
    const VENDOR_DEFAULT = 'slothsoft';
 ...
 // XSLTProcessor
    ...
    public function init($siteMapPath)
    {
        ...
        $this->requestedVendor = self::VENDOR_DEFAULT;
        $this->requestedModule = self::MODULE_DEFAULT;
        
        // $this->sitesInitialized = false;
        // $this->resourceDocs['/core/sites'] = $this->sitesDoc;
        //$this->moduleRoot = $this->sitesDoc->createElementNS(DOMHelper::NS_FARAH_MODULE, self::TAG_MODULES);
    }
    ...
    private function lookupDomain($domain)
    {
        ...
        /* @TODO: allow references in sites doc
    ...
    private function loadPage()
    {
        ...
        if ($this->requestedDomain and $this->requestedPage) {
            $this->requestedVendor = $this->getOwnerVendor($this->requestedPage);
            $this->requestedModule = $this->getOwnerModule($this->requestedPage);
            ...
            if ($this->requestedPage->hasAttribute('ref')) {
                $this->httpResponse->addHeader('content-location', $this->findUri($this->requestedPage));
                ...
                $module = $repository->lookupModule($this->requestedVendor, $this->requestedModule);
                $uri = AssetUri::createFromReference($ref, $module, $args);
                $asset = $repository->lookupAsset($uri);
                
                //echo "loaded page {$asset->getId()}, processing..." . PHP_EOL;
                switch (true) {
                    case $asset instanceof DOMWriter:
                        return $asset->toNode();
                    case $asset instanceof FileWriter:
                        return $asset->toFile();
                    default:
                        throw new RuntimeException("asset {$asset->getId()} must implement a Writer-type thingamabob!");
                }
            }
        }
        return null;
    ...
    private function loadPTDocument()
    {
        ...
        $repo = PTRepository::getInstance($module);
    ...
    private function getOwnerVendor(DOMElement $node)
    {
        $ret = $this->sitesPath->evaluate('string(ancestor-or-self::*[@vendor][1]/@vendor)', $node);
        return strlen($ret) ? $ret : self::VENDOR_DEFAULT;
    }
